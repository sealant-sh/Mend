...
- `fonts/` vendors the symbols-only Nerd Font (MIT) the surface registers lazily, so prompt glyphs
...
- `vendor/` holds only the artifacts, reproducibly generated by
  ...
  `native/libghostty-vt/` at the repository root; the wasm embeds the pinned revision in its build
  info and the ABI test verifies it against mobile's `VERSION`.
