...
- `apps/docs/src/content/docs/getting-started/requirements.md` at `/getting-started/requirements/`
  is **[Shipped] Requirements**, covering Linux, x64 and arm64, `curl`, `git`, Docker daemon access,
  Docker Compose 2.23.1 or newer, network trust, and disk prerequisites verified against
  `install.sh`.
- `apps/docs/src/content/docs/getting-started/install.md` at `/getting-started/install/` is
  **[Shipped] Install Mend**, covering the inspect-and-run installer path, account creation,
  `mend login`, account connection, `mend doctor`, and post-install health checks.
...
| Installer behavior                           | `install.sh`                                                                                                                                                                  | `/getting-started/requirements/`, `/getting-started/install/`, `/operations/*`, `/reference/configuration/` | Every command, path, port, version, and default must match an installer smoke test.                      |
...
| Production topology                          | `install.sh`                                                                                                                                                                  | `/operations/*` and `/architecture/system-overview/`                                                        | Root `compose.yaml` is an alternative and remains experimental until its end-to-end mount test passes.   |
...
| Runtime configuration                        | Typed registry to be added, with current reads in store, sessions, auth, inference, database tests, `install.sh`, and Compose files as migration inputs                       | `/reference/configuration/` and contributor environment page                                                | CI rejects a key present in code but absent from the registry and reference.                             |
...
| Release and operations | `install.sh`, `compose.yaml`, `compose.dev.yaml`, installer workflows, and `apps/docs/src/content/docs/operations`                        | Port, path, service name, minimum version, install, upgrade, backup, restore, uninstall, or bind change.                              |
...
| `install.sh`, `compose.yaml`, or `compose.dev.yaml`                                                    | Regenerate or review requirements, install, secure-instance, operations, configuration, system overview, and release pages.                     |
...
10. Keep the byte-identity check between `install.sh` and `apps/marketing/public/install.sh`; add
    `sh -n`, ShellCheck, `docker compose config`, and production-secret policy checks.
...
| Supported backup and restore                                                                                             | Planned until tested                           | Clean restore of Mend Postgres, Sealant Postgres, store, configuration, keys, and Sealant records with version and integrity checks.                          | Keep `/operations/backup-and-restore/` labeled Planned and do not promise recoverability.                      |
| Supported upgrade and rollback                                                                                           | Planned until tested                           | Version compatibility matrix, pre-upgrade backup, migration, health, failure, and rollback-limit acceptance runs.                                             | State that installer reruns preserve existing volumes but run migrations without automatic backup or rollback. |
...
| Root `compose.yaml` as a supported production topology                                                                   | Experimental                                   | End-to-end store mount, Sealant reachability, secret, restart, upgrade, backup, and workspace launch tests.                                                   | Name the `install.sh` topology as supported and label root Compose experimental.                               |
